...
import { assinaturaAtom, carrinhoAtom, loginAtom, saldoDisponivelAtom, saldoPendenteAtom, showCarrinhoAtom } from '../../../context/jotai';
...
const Carrinho = ({isOpen, closeCarrinho}) => {
  const [saldoDisponivel, setSaldoDisponivelAtom] =  useAtom(saldoDisponivelAtom);
  const [saldoPendente, setSaldoPendenteAtom] =  useAtom(saldoPendenteAtom);
  const [login, setLoginAtom] =  useAtom(loginAtom);
  const [assinaturaSelecionada, setAssinaturaSelecionada] = useAtom(assinaturaAtom);
  const [listaCarrinho, setListaCarrinho] = useAtom(carrinhoAtom);
  const [showCarrinho, setShowCarrinho] = useAtom(showCarrinhoAtom);
