...
export const limparFiltros = (filtros) => {
  filtros.forEach(({ setter, ref }) => {
    if (setter) setter({ target: { value: "" } });
...
export const handleResizeTabela = debounce((tabelaId, container) => {
  ...
  const sidebarColumn = mainContainer.parentElement.previousElementSibling;
  const fullScreenWidth = mainContainer.parentElement.parentElement.offsetWidth - 24 - sidebarColumn?.offsetWidth;
...
export const ordenarArray = (array, key) => {
